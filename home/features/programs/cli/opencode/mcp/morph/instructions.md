...
**Prefer `warpgrep_github_search` over web search or docs fetching** when the
question is about how an open-source library or SDK works internally. If docs
URLs return 404s or you need implementation-level understanding, go to the
source.
...
- If `warpgrep_github_search` fails, clone the repo only if the task justifies
  local setup cost
...
- The tool descriptions are self-contained, but loading this file as an
  always-on instruction makes tool choice more reliable
