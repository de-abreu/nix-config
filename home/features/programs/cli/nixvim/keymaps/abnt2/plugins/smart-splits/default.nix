...
{
  programs = {
    nixvim.keymaps = lib.mkIf cfg.enable (resize-windows ++ move-cursor ++ swap-buffers);

    # Wezterm integration untouched
    wezterm.extraConfig."keybinds.smart-splits" = lib.mkIf cfg.enable ./smart-splits.lua;
  };
}
